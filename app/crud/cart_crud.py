...
class CartCRUD(CRUDBase[Cart]):
    ...
    def merge_guest_cart(
        ...
        for guest_item in guest_items:
            # 동일 SKU의 회원 장바구니 아이템 찾기
            existing_item = self._get_cart_item_crud().get_by_cart_and_sku(
                db, member_cart.id, guest_item.sku_id
            )
    ...
    def _get_cart_item_crud(self) -> CartItemCRUD:
        """CartItemCRUD 인스턴스를 반환한다. (순환 참조 방지용 지연 임포트)"""
        from app.crud.cart_crud import CartItemCRUD
